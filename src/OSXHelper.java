...
public class OSXHelper {
    ...
    static protected final File THIS_JAR_FILE = getThisJarFile(new Object() {}.getClass().getEnclosingClass());
    ...
    static private File getThisJarFile(final Class aclass) {
        ...
        try {
            ...
            if (!success) {
                ...
                try { // to get full path if aclass.getResource().getPath() returned just the resource name
                ...
                if (!success) { // Maybe aclass.getResource().getPath() returned the full path initially
                    try { // to decode it
                        decoded = new URL(path).toURI().getPath(); //URLDecoder.decode(path, "UTF-8");
                        success = true;
                    } catch (final Exception e) {
        ...
        try {
            ...
            if (!XdockSet) {
                if (applicationIcon != null) {
                    iconPath = exportResource(new Object() {}.getClass().getEnclosingClass(), applicationIcon);
                }
                ...
                invokeLater(() -> {
                    if (applicationIcon != null) {
                        try {
                            ...
                            String iconPath1 = exportResource(new Object() {}.getClass().getEnclosingClass(), applicationIcon);
                            Thread.sleep(1000);
